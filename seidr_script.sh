...
OPTIONS:
 ...
 -m	multiple raw_counts.txt files provided (must be of same sample lenght)
...
With the -t option it is possible to specify a gene list to generate a Targeted Network. This new network will only show the gene nodes present in the list and the edges between them and between other genes (not specified in the list) if they happen to be co-expressed.

#Multiple Counts.txt tables can be provided with the -m option, with the requesite that they must have the same sample size and in same order.#

###
My work uses a Raw_Counts.txt table generated by the Raw_Counts.R script which includes the gene counts for 74 samples from 6 BioProjects sample data.
###
...
#if mcounts="True"
#then
	#Raw_Counts.R --tables f --out Raw_Counts.txt
	#This script joins the counts from separate tables into one global Raw_Counts.txt table (example: Project1_raw_counts.txt Project2_raw_counts.txt ...)
#fi
